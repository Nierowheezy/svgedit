...
import { convertAttrs } from "../common/units.js";
import {
  transformPoint,
  hasMatrixTransform,
  getMatrix,
...
const getBsplinePoint = t => {
  ...
  const p0 = {
    x: svgCanvas.getControllPoint2("x"),
    y: svgCanvas.getControllPoint2("y")
  };
  const p1 = {
    x: svgCanvas.getControllPoint1("x"),
    y: svgCanvas.getControllPoint1("y")
  };
...
const mouseMoveEvent = evt => {
  ...
  switch (svgCanvas.getCurrentMode()) {
    ...
    case "resize": {
      ...
      if (
        !svgCanvas.getCurrentResizeMode().includes("n") &&
        !svgCanvas.getCurrentResizeMode().includes("s")
      ) {
        dy = 0;
      }
      if (
        !svgCanvas.getCurrentResizeMode().includes("e") &&
        !svgCanvas.getCurrentResizeMode().includes("w")
      ) {
        dx = 0;
      }
    ...
    case "image": {
      ...
      if (square) {
        w = h = Math.max(w, h);
        newX =
          svgCanvas.getStartX() < x
            ? svgCanvas.getStartX()
            : svgCanvas.getStartX() - w;
        newY =
          svgCanvas.getStartY() < y
            ? svgCanvas.getStartY()
            : svgCanvas.getStartY() - h;
      } else {
    ...
    case "fhellipse":
    case "fhrect": {
      svgCanvas.setFreehand(
        "minx",
        Math.min(realX, svgCanvas.getFreehand("minx"))
      );
      svgCanvas.setFreehand(
        "maxx",
        Math.max(realX, svgCanvas.getFreehand("maxx"))
      );
      svgCanvas.setFreehand(
        "miny",
        Math.min(realY, svgCanvas.getFreehand("miny"))
      );
      svgCanvas.setFreehand(
        "maxy",
        Math.max(realY, svgCanvas.getFreehand("maxy"))
      );
    }
    // Fallthrough
    case "fhpath": {
      ...
      if (
        svgCanvas.getControllPoint2("x") &&
        svgCanvas.getControllPoint2("y")
      ) {
        for (i = 0; i < svgCanvas.getStepCount() - 1; i++) {
          ...
          svgCanvas.setNextPos({
            x: svgCanvas.getbSpline("x"),
            y: svgCanvas.getbSpline("y")
          });
          svgCanvas.setbSpline(getBsplinePoint(svgCanvas.getParameter()));
          svgCanvas.setSumDistance(
            svgCanvas.getSumDistance() +
              Math.sqrt(
                (svgCanvas.getNextPos("x") - svgCanvas.getbSpline("x")) *
                  (svgCanvas.getNextPos("x") - svgCanvas.getbSpline("x")) +
                  (svgCanvas.getNextPos("y") - svgCanvas.getbSpline("y")) *
                    (svgCanvas.getNextPos("y") - svgCanvas.getbSpline("y"))
              )
          );
          if (svgCanvas.getSumDistance() > svgCanvas.getThreSholdDist()) {
            svgCanvas.setSumDistance(
              svgCanvas.getSumDistance() - svgCanvas.getThreSholdDist()
            );
      ...
      svgCanvas.setStart({
        x: svgCanvas.getEnd("x"),
        y: svgCanvas.getEnd("y")
      });
      break;
...
const mouseUpEvent = evt => {
  ...
  switch (svgCanvas.getCurrentMode()) {
    ...
    case "select":
      if (selectedElements[0]) {
        ...
        if (!selectedElements[1]) {
          ...
          switch (selected.tagName) {
            ...
            case "foreignObject":
              break;
            case "text":
              svgCanvas.setCurText(
                "font_size",
                selected.getAttribute("font-size")
              );
              svgCanvas.setCurText(
                "font_family",
        ...
        if (
          realX !== svgCanvas.getRStartX() ||
          realY !== svgCanvas.getRStartY()
        ) {
    ...
    case "fhellipse":
      if (
        svgCanvas.getFreehand("maxx") - svgCanvas.getFreehand("minx") > 0 &&
        ...
        element = svgCanvas.addSVGElementsFromJson({
          ...
          attr: {
            cx:
              (svgCanvas.getFreehand("minx") + svgCanvas.getFreehand("maxx")) /
              2,
            cy:
              (svgCanvas.getFreehand("miny") + svgCanvas.getFreehand("maxy")) /
              2,
            rx:
              (svgCanvas.getFreehand("maxx") - svgCanvas.getFreehand("minx")) /
              2,
            ry:
              (svgCanvas.getFreehand("maxy") - svgCanvas.getFreehand("miny")) /
              2,
            id: svgCanvas.getId()
          }
        });
  ...
  if (!keep && element) {
    ...
    if (
      (svgCanvas.getCurrentMode() !== "path" || !svgCanvas.getDrawnPath()) &&
      t &&
